    ...
    if data.permissions['delete_note'] == '1'
      $.each $('.note_panel'), (i, val) ->
        ...
        $.each $("##{id} #note_menu"), (j, value) ->
          ...
          if $("##{id} #note_menu .fa-times").val() == undefined
            $(value).prepend("<li id='destroy_note_board_id_#{getBoardId()}_user_id_#{data.user_id}'><a data-toggle='modal' title='delete note' href='#delete_#{note_id}'><div class='fa fa-times'></div></a></li>")
            return
